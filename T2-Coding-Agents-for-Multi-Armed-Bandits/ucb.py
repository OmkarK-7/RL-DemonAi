...
class UCBAgent(Agent):
    ...
    def __init__(self, time_horizon, bandit:MultiArmedBandit,): 
        ...
        self.counts = np.zeros(len(self.bandit.arms))  # Number of times each arm was pulled
        self.values = np.zeros(len(self.bandit.arms))  # Estimated value of each arm
        self.time = 0

    def give_pull(self):
        chosen_arm = np.argmax([self.values(arm) for arm in range(len(self.bandit.arms))])
    ...
    def plot_arm_graph(self):
        plt.bar(range(len(self.bandit.arms)), self.values)
        plt.xlabel('Arms')
